...
app.config['MAIL_USERNAME'] = 'abhaykaul0101@gmail.com'
app.config['MAIL_PASSWORD'] = 'Kaul@1234'
...
def text_pdf():
    ...
    print('here==============')
    # recep_email = request.data['email']
    recep_email = 'kaulabhay673@gmail.com'
    ...
    df_words_perso = pd.read_csv('static/js/db/words_perso.txt', sep=',', error_bad_lines=False)
    msg = Message(
                sender ='abhaykaul0101@gmail.com',
                recipients = [request.form.get('email')]
               )
    # msg.html = render_template('text_dash.html', traits = probas, trait = trait, trait_others = trait_others, probas_others = probas_others, num_words = num_words, common_words = df_words_perso, common_words_others=common_words_others)
    msg.html = render_template('text_dash.html', traits = probas, trait = trait, trait_others = trait_others, probas_others = probas_others, num_words = num_words, common_words = df_words_perso, common_words_others=common_words_others)
    #print(msg,mail)
    with open('./static/CSS/images/text_head.png', 'rb') as fp:
        msg.attach('text_head.jpg', 'image/jpg', fp.read(), 'inline', headers=[['Content-ID','<text_head>']])

    mail.send(msg)
    return render_template('text_dash.html', traits = probas, trait = trait, trait_others = trait_others, probas_others = probas_others, num_words = num_words, common_words = df_words_perso, common_words_others=common_words_others)
    return "<script>alert('We got your interview, Thankyou !')</script>";
